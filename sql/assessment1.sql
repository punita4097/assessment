/*Ques 1.  Create a table (datatype should be appropriate for each column).Insert data into table,Update one table data using join with other table 
(you can create another table   which takes the reference from the first table). 
*/
...
/*  data entry in the employee table*/
...
select sum(salary)from record r
